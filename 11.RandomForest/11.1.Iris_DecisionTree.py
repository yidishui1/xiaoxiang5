...
if __name__ == "__main__":
    ...
    print u'测试集准确率：', accuracy_score(y_test, y_test_hat)
    ...
    cm_light = mpl.colors.ListedColormap(['#68228B', '#FFA0A0', '#FFA500'])
    cm_dark = mpl.colors.ListedColormap(['g', 'r', 'b'])
    y_show_hat = model.predict(x_show)  # 预测值
    print 'y_show_hat.shape:',y_show_hat.shape
    print 'y_show_hat:',y_show_hat
    y_show_hat = y_show_hat.reshape(x1.shape)  # 使之与输入的形状相同
    print 'y_show_hat:',y_show_hat
    plt.figure(facecolor='w')
    plt.pcolormesh(x1, x2, y_show_hat, cmap=cm_light)  # 预测值的显示
    plt.contour(x1,x2, y_show_hat, levels=(0,1),colors='r',linestyle='-.')
    # plt.contourf(x1, x2, y_show_hat)
