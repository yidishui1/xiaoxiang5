...
def read_data():
    ...
    plt.title('实际排放数据0904', fontsize=15)
    plt.legend(loc='upper right')
    plt.grid(b=True, ls=':', color='#202020')

    plt.subplot(122)
    data = pd.read_csv('..\\C0911.csv', header=0)
    x = data['H2O'].values
    plt.plot(x, 'r-', lw=1, label='C0911')
    plt.title('实际排放数据0911', fontsize=15)
    plt.legend(loc='upper right')
    plt.grid(b=True, ls=':', color='#202020')
...
if __name__ == "__main__":
    ...
    plt.plot(abnormal, x[abnormal], 'go', markeredgecolor='g', ms=3, label='异常值')
    plt.legend(loc='upper right')
    plt.title('异常检测', fontsize=15)
    plt.grid(b=True, ls=':', color='#202020')
