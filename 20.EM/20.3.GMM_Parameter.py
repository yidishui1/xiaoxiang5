...
if __name__ == '__main__':
    ...
    for i, type in enumerate(types):
        ...
        bic[i] = gmm.bic(x)
    print u'错误率：', err.ravel()
    print u'BIC：', bic.ravel()
    xpos = np.arange(4)
    # print 'xpos:',xpos
    ...
    plt.title(u'不同方差类型的误差率和BIC', fontsize=15)
    plt.show()
    ...
    # print 'gmm.means_[0][0] , gmm.means_[1][0]:',gmm.means_[0][0] , gmm.means_[1][0]
    if gmm.means_[0][0] > gmm.means_[1][0]:
        z = grid_hat == 0
        grid_hat[z] = 1
        grid_hat[~z] = 0
        # print 'z:',z
        # print 'grid_hat[z]',grid_hat[z]
        # print 'grid_hat[~z]', grid_hat[~z]
    # print 'grid_hat:',grid_hat
    plt.figure(facecolor='w')
    ...
    plt.title(u'GMM调参：covariance_type=%s' % types[optimal], fontsize=15)
    plt.grid(b=True, ls=':', color='#606060')
    plt.tight_layout(2)
    plt.show()
