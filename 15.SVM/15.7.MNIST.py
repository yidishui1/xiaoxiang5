...
if __name__ == "__main__":
    ...
    print u'图片个数：%d，图片像素数目：%d' % x.shape
    ...
    t = time() - t
    print u'%s训练结束，耗时%d分钟%.3f秒' % (classifier_type, int(t/60), t - 60*int(t/60))
    t = time()
    y_train_pred = model.predict(x_train)
    t = time() - t
    print u'%s训练集准确率：%.3f%%，耗时%d分钟%.3f秒' % (classifier_type, accuracy_score(y_train, y_train_pred)*100, int(t/60), t - 60*int(t/60))
    t = time()
    y_valid_pred = model.predict(x_valid)
    t = time() - t
    print u'%s测试集准确率：%.3f%%，耗时%d分钟%.3f秒' % (classifier_type, accuracy_score(y_valid, y_valid_pred)*100, int(t/60), t - 60*int(t/60))
    save_result(model)
    ...
    for index in range(12):
        ...
        plt.imshow(image.values.reshape(28, 28), cmap=plt.cm.gray_r, interpolation='nearest')
        plt.title(u'错分为：%i，真实值：%i' % (err_y_hat[index], err_y.values[index]), fontsize=12)
